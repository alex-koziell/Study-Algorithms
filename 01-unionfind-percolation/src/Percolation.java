...
public class Percolation {
    ...
    */
    ...
    public Percolation(int n) {
        ...
        wquf = new WeightedQuickUnionUF(numSites+2); // 2 additional 'top' [numSites] and 'bottom' [numsites+1] nodes for faster algorithm
        siteValues  = new boolean[numSites];
    ...
    public boolean isOpen(int row, int col) {
        ...
        return siteValues[row*gridWidth+col];
    }
    ...
    public boolean isFull(int row, int col) {
        if (row>=gridWidth || col>=gridWidth)
            throw new IllegalArgumentException("Index out of range.");
        return (wquf.find(row*gridWidth+col)==wquf.find(numSites));
    }
